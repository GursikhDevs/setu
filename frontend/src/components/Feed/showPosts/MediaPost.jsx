...
import { Heart, ThumbsUp, Lightbulb, Sparkles, Trophy, Play } from 'lucide-react';
...
const MediaPost = ({ post }) => {
  ...
  const isVideo = post?.media?.type === 'video' || post?.media?.url?.match(/\.(mp4|webm|ogg)$/i);

  return (
    <article className="w-[95%] max-w-2xl h-fit mx-auto shrink-0 bg-main-color text-theme-white border border-slate-200 rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg hover:border-slate-300 shadow-sm shadow-theme-white">
      <div className="w-full h-fit p-3 md:px-6 pb-0 ">
        ...
        {post?.text && (
          <div className="prose prose-slate w-full mb-2  ">
            <p className=" text-xs leading-4 whitespace-pre-wrap">
              {showFull ? post?.text : post?.text?.slice(0, 100) + "..."}
      ...
      <div className="relative w-full h-fit group flex items-center justify-center">
        {isVideo ? (
          <div className="relative aspect-video bg-slate-900">
            <video
              src={post?.media?.url || ''}
              className="w-auto h-full max-h-70 object-contain"
          ...
          <img
            src={post?.media?.url || '/images/walk.gif'}
